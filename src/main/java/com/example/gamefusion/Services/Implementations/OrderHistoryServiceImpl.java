...
public class OrderHistoryServiceImpl implements OrderHistoryService {
    ...
    public OrderHistoryDto findById(Long id) {
        ...
        throw new EntityNotFound("OrderHistory entity with this id is not present");
    }

    @Override
    public List<OrderHistoryDto> findByOrderId(Integer orderId) {
        if (!orderMainService.isExistsByID(orderId))
            throw new EntityNotFound("OrderMain entity with this OrderId is not present");
        if (!isExistsByOrderId(orderId))
            throw new EntityNotFound("OrderHistory entity with this OrderId is not present");
    ...
    public OrderHistoryDto save(OrderHistoryDto orderHistoryDto) {
        return conversionUtil.entityToDto(
            repository.save( conversionUtil.dtoToEntity( orderHistoryDto ) )
        );
    }
